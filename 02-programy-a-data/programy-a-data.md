...
> Když počítač vypneme, programy ani dokumenty „nezmizí“. Zůstávají uložené jako soubory, které operační systém organizuje, chrání a zpřístupňuje aplikacím. Nad touto zdánlivě jednoduchou vrstvou pak stojí celý svět operačních systémů, licencí, virtualizace, cloudových služeb, aktualizací a záloh.

Tento text modernizuje původní výukový materiál **Programy a data**. Zachovává jeho hlavní témata — soubory a adresáře, systémový a aplikační software, licence, instalaci programů, virtualizaci, cloud a zálohování — ale přesouvá je do šesti didakticky ucelených lekcí. Některé historické formulace jsou zpřesněny a několik oblastí je rozšířeno tak, aby odpovídaly dnešní praxi: správě balíčků, UEFI, kontejnerům, cloudovým modelům, synchronizaci, pravidlu 3–2–1 a bezpečné správě softwaru.

Text je psán jako souvislý výklad, nikoli jako slovník pojmů. Jednotlivé kapitoly proto používají konkrétní příklady, analogie a krátké technické scénáře. Díky tomu mohou později sloužit také jako podklad pro komentované prezentace nebo podcastové minipořady.

---

# 1. Soubory, adresáře a data na disku

## 1.1 Soubor: pojmenovaný balíček dat

Když uložíme dokument, fotografii nebo program, operační systém musí vědět, kde data leží, jak jsou velká, komu patří a jak s nimi smí zacházet. Základní jednotkou takové organizace je **soubor**.

Soubor můžeme chápat jako pojmenovaný celek dat uložený v souborovém systému. Může obsahovat text, obraz, zvuk, databázová data, programový kód, konfiguraci nebo téměř libovolný jiný typ informace. Pro operační systém je soubor především posloupnost bajtů doplněná o metadata.

K nejběžnějším metadatům patří:
...
- přístupová práva,
- případně další atributy specifické pro daný souborový systém.

Uživatel obvykle pracuje s názvem, ale operační systém uvnitř používá také vlastní identifikátory. To je důležité například při přejmenování: změní se jméno, nikoli nutně fyzické umístění všech dat na disku.

Název souboru může obsahovat také **příponu**, například `.txt`, `.jpg`, `.pdf` nebo `.py`. Přípona je praktická pomůcka pro člověka i systém a často se používá k asociaci souboru s určitou aplikací. Není však sama o sobě zárukou skutečného obsahu.

Soubor přejmenovaný z `obrazek.jpg` na `obrazek.txt` se automaticky nestane textem. Změnili jsme jen část názvu. Skutečný formát určují data uvnitř souboru.

Moderní systémy proto mohou využívat i další informace, například **MIME type**, metadata nebo charakteristické bajtové signatury na začátku souboru. Některým takovým signaturám se neformálně říká *magic numbers*.

Příklad: PNG soubor má na začátku charakteristickou bajtovou sekvenci, podle které jej lze rozpoznat i tehdy, když nemá příponu `.png`.

To má také bezpečnostní význam. Soubor pojmenovaný `faktura.pdf.exe` může při skrytých příponách vypadat jako dokument, ale ve skutečnosti jde o spustitelný soubor. Při práci se staženými přílohami je proto vhodné zobrazovat skutečné přípony a nespoléhat pouze na ikonu.

**Hlavní myšlenka:** soubor není „typ podle přípony“. Je to pojmenovaná datová entita, jejíž význam vzniká kombinací obsahu, formátu, metadat a způsobu interpretace.

---

## 1.2 Adresáře, cesty a strom souborového systému

Jakmile máme stovky nebo miliony souborů, potřebujeme je organizovat. K tomu slouží **adresáře**, v grafickém prostředí často nazývané složky.

Adresáře obvykle vytvářejí hierarchickou strukturu připomínající strom. Na jeho vrcholu stojí kořen a pod ním další adresáře a soubory.

V Unixu a Linuxu je kořen souborového systému označen:

`/`

Například:

`/home/student/projekty/web/index.html`

Ve Windows má cesta často podobu:

`C:\Users\Student\Documents\projekt\index.html`

Windows používá písmena jednotek a tradičně zpětné lomítko, zatímco unixové systémy pracují s jediným logickým stromem a běžným lomítkem.

Rozlišujeme **absolutní a relativní cestu**.

Absolutní cesta popisuje umístění od jednoznačného počátku:

`C:\Users\Student\Documents\data.csv`

Relativní cesta se vztahuje k aktuálnímu pracovnímu adresáři:
...
Zápis `..` obvykle znamená nadřazený adresář, `.` aktuální adresář.

Relativní cesty jsou velmi důležité při programování a tvorbě webů. Projekt lze přenést na jiný počítač, a pokud používá správně navržené relativní odkazy, nemusíme všude měnit pevné absolutní cesty.

Při práci v příkazovém řádku se často používají také **zástupné znaky**, anglicky *wildcards*. Jejich přesný význam závisí na shellu a programu, ale tradičně:

- `*` zastupuje libovolnou posloupnost znaků,
- `?` zastupuje jeden znak.

Například příkaz pracující s `*.jpg` může vybrat všechny soubory s příponou `.jpg` v daném adresáři.

Strom adresářů je logická struktura. Fyzické bloky jednoho souboru nemusí na disku ležet vedle sebe. O jejich umístění se stará **souborový systém**, například NTFS, ext4, APFS nebo exFAT.

Souborový systém vede evidenci, které bloky patří kterému souboru, spravuje názvy, metadata, přístupová práva a volné místo. Je tedy jakousi „evidencí skladu“, která umožňuje operačnímu systému najít správná data.

**Příklad z praxe.** Když ve správci souborů přetáhneme soubor mezi dvěma adresáři na stejném svazku, nemusí se fyzicky přepisovat celý obsah. Souborový systém může změnit především záznam o jeho umístění v adresářové struktuře. Přesný postup ale závisí na konkrétním systému a situaci.

---

## 1.3 Programové a datové soubory

Tradiční dělení rozlišuje **programové a datové soubory**. Je užitečné, ale dnešní počítačové systémy ukazují, že hranice není vždy tak ostrá, jak se zdá.

Programový soubor obsahuje instrukce nebo popis postupu, který lze nějakým prostředím vykonat. Datový soubor naopak obvykle obsahuje informace, které program zpracovává.

Například:

- `editor.exe` — spustitelný program,
- `skript.py` — zdrojový nebo skriptovací program,
- `dokument.docx` — dokumentová data,
- `foto.jpg` — obrazová data,
- `data.csv` — tabulková data.

Spustitelné programy mohou být přeložené do strojového kódu pro konkrétní procesorovou architekturu a operační systém. Jiné programy jsou distribuovány jako mezikód nebo jako textový zdroj, který zpracuje interpreter či virtuální stroj.

Skript například může být obyčejný textový soubor. To ukazuje, že označení „textový“ a „programový“ nejsou protiklady. Pythonový soubor `.py` je textový soubor a zároveň obsahuje programové instrukce.

Stejně zajímavé jsou dokumenty obsahující makra. Tabulkový soubor může nést data i programové instrukce. Webová stránka obsahuje HTML a CSS jako data pro prohlížeč, ale může obsahovat také JavaScript, který se vykonává.

Proto je přesnější uvažovat o **roli souboru v konkrétním kontextu** než o absolutní hranici.

Program také obvykle netvoří jediný soubor. Může používat:

- spustitelný soubor,
- knihovny,
- konfigurační soubory,
- jazykové balíčky,
- ikony a další zdroje,
- databáze,
- cache,
- uživatelská data.

Moderní aplikace mohou být dokonce složené ze stovek nebo tisíců souborů.

Důležité je také rozlišit **program na disku** a **proces v paměti**. Program je uložená sada instrukcí a dalších dat. Když jej operační systém spustí, vytvoří běžící instanci — proces. Jeden program může mít současně několik procesů a jeden proces může používat mnoho dalších souborů.

**Příklad.** Prohlížeč uložený na disku je program. Po jeho spuštění může operační systém vytvořit hlavní proces a další procesy pro jednotlivé karty, rozšíření, vykreslování nebo GPU. To, co uživatel vnímá jako „jeden Chrome“ nebo „jeden Firefox“, je uvnitř celý soubor spolupracujících procesů.

---

## 1.4 Spustitelné soubory, skripty a knihovny

Program může být distribuován různými způsoby. Nejjednodušší představa je spustitelný binární soubor, ale moderní software využívá více vrstev.

Ve Windows se často setkáme s `.exe`, v unixových systémech nemusí spustitelný soubor mít žádnou specifickou příponu. Rozhodující mohou být přístupová práva, formát souboru a informace v jeho hlavičce.

Skriptovací soubor obsahuje instrukce pro program, který je interpretuje nebo předává dalšímu prostředí. Typické příklady:
...
V unixových systémech může první řádek skriptu obsahovat takzvaný **shebang**, například:

`#!/usr/bin/env python3`

Ten systému napovídá, jaký interpreter má soubor spustit.

Velké programy často využívají **knihovny**, tedy znovupoužitelný kód poskytující hotové funkce. Knihovny mohou být připojeny staticky při sestavení programu nebo dynamicky načítány při běhu.

Ve Windows se běžně setkáme s knihovnami DLL, v Linuxu například se soubory `.so`, v macOS s dynamickými knihovnami a frameworky.

Dynamické knihovny přinášejí výhodu sdílení: více programů může používat stejnou implementaci určité funkce. Zároveň však vzniká závislost na správné verzi knihovny.

Odtud pochází známý problém **dependency hell** — aplikace potřebuje určitou verzi knihovny, jiná aplikace jinou a obě očekávání se mohou dostat do konfliktu.

Moderní balíčkové systémy a izolovaná prostředí se snaží tyto závislosti spravovat automaticky.

Na rozdíl od starší představy tedy „program“ není vždy jeden soubor, který pouze nakopírujeme na disk. Je to často síť závislostí mezi binárními soubory, knihovnami, konfigurací a daty.

---

## 1.5 Typ souboru, asociace a bezpečnost

Operační systém může asociovat určitý typ souboru s konkrétní aplikací. Po poklepání na `.pdf` se otevře prohlížeč PDF, `.jpg` může otevřít editor obrázků a `.docx` textový procesor.

Asociace ale neurčuje skutečný obsah. Je to pravidlo typu:

**„když uživatel otevře soubor s tímto typem, nabídni tento program.“**

U některých systémů je typ rozpoznáván podle přípony, jinde hrají větší roli metadata nebo interní struktura souboru.

To je důležité pro kompatibilitu. Program může podporovat více formátů a jeden formát může otevřít více různých aplikací.

Například `.png` můžeme zobrazit v prohlížeči, grafickém editoru i programu pro správu fotografií. Soubor není „majetkem Photoshopu“ nebo „majetkem prohlížeče“.

Při přenosu dat přes web se používají například **MIME media types**, jako:
...
Ty pomáhají určit, jak má klient obsah interpretovat.

Bezpečnostní problém vzniká tehdy, když uživatel zamění vzhled souboru za jeho skutečný typ. Útočník může použít dvojitou příponu, podvrženou ikonu nebo dokument s aktivním obsahem.

Proto je vhodné:

- zobrazovat přípony známých typů,
- nestahovat spustitelné soubory z nedůvěryhodných zdrojů,
- kontrolovat digitální podpisy instalátorů,
- používat aktuální ochranné mechanismy operačního systému,
- nepovažovat soubor za bezpečný jen proto, že se tváří jako dokument.

**Hlavní myšlenka první lekce:** souborový systém vytváří logickou vrstvu nad fyzickým úložištěm. Programy i data jsou soubory nebo skupiny souborů a jejich význam nevzniká pouze podle názvu, ale podle obsahu, formátu, oprávnění a způsobu použití.
